...
def get_data(dataset):
    if dataset == "fmnist":
        train = FashionMNIST(root='/home/lishiwei/fedsrn/dataset/', train=True, download=True)
    if dataset == "svhn":
        train = SVHN(root='/home/lishiwei/fedsrn/dataset/', split='train', download=True)
    if dataset == "cifar10":
        train = CIFAR10(root='/home/lishiwei/fedsrn/dataset/', train=True, download=True)
    if dataset == "cifar100":
        train = CIFAR100(root='/home/lishiwei/fedsrn/dataset/', train=True, download=True)
    return train
...
def partition_train_data(dataset, y, part_strategy, num_clients, seed=1234):
    ...
    os.makedirs("/home/lishiwei/fedsrn/dataloader/_npy_/", exist_ok=True)
    np.save("/home/lishiwei/fedsrn/dataloader/_npy_/"+npy_name, net_dataidx_map)
...
if __name__ == "__main__":
    ...
    datasets=["cifar100"]
    part_strategies=["iid", "labeldir0.2", "labelcnt0.2"]
    for dataset in datasets:
        for part_strategy in part_strategies:
            for client in clients:
                generate_train_npy(dataset, part_strategy, client, seed)