...
class SegmentHead(nn.Module):
    def __init__(self, in_chan, mid_chan, n_classes, up_factor=8, aux=True):
        ...
        self.conv_out = nn.Sequential(
            ...
            nn.Conv2d(mid_chan2, out_chan, 1, 1, 0, bias=False),     # todo
    ...
    def __init__(self, dim_in, proj_dim=256, proj='convmlp', ):
        ...
        if proj == 'linear':
            ...
            self.proj = nn.Sequential(
                nn.Conv2d(dim_in, dim_in, kernel_size=1, bias=False), # todo
                nn.BatchNorm2d(dim_in),
                nn.ReLU(inplace=True),
                nn.Conv2d(dim_in, proj_dim, kernel_size=1, bias=False) # todo
            )
    def forward(self, x):
...
class BiSeNetV2(nn.Module):
    ...
    def load_pretrain(self):
        ...
        state = torch.load('./myseg/backbone_v2.pth')  # baidu server
