...
from model.srn import srn_set_parameters, srn_get_parameters
...
class Client(fl.client.NumPyClient):
    def __init__(self, args):
        ...
        if self.com_type == "fedsrn":
            self.keys_server = [item for item in self.keys_client if 'update' not in item and 'noise' not in item]
    ...
    def set_parameters(self, parameters, config):
        if self.com_type == 'fedavg':
            ...
            self.model.load_state_dict(state_dict, strict=True)
            srn_set_parameters(self.model, config)
    ...
    def get_parameters(self, config):
        ...
        if self.com_type == 'fedsrn':
            srn_get_parameters(self.model)
            state_dict = copy.deepcopy(self.model.state_dict())
            for key in self.keys_weight:
                del state_dict[key+'update']
                del state_dict[key+'noise']
            return [val.cpu().numpy() for _, val in state_dict.items()]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="cnn4", help="model")
    ...
    parser.add_argument("--num_client", type=int, default=30, choices=range(2, 300), help="num_client")
    parser.add_argument("--id", type=int, default=9, choices=range(0, 299), help="client id")
